...
typedef struct client
{
	...
	int num_boisson; //choix de boisson du client
...
int commande(CLIENT T);//Fonction qui va classer le client dans un thread approprié selon la boisson qu'il a choisi, décompte le temps de préparation de la boisson et qui renvoie un 1 lorsque le client est servi
//Priorité 2
	...
	switch(rep)
		{
			...
			case 2 :	//printf("\033c");
					...
					break;
			case 3 :
					printf("Donnez le numero du client qui part du bar\n");
					scanf("%d",&lq);
					p=suppr_client(&T,lq);
...
int menu(void)
{
	...
	printf("\t --5: Servir le brave bougre \n");
...
void aff_noeud(CLIENT *N){
		printf("[NUM:%3d  |BOISSON:%d  |NB_VERRE:%d  |ETAT:%d  |ARRIVEE:%2d %2d:%2d]\n",N->num_client,N->num_boisson,N->nb_verre,N->etat,N->arrive.jour,N->arrive.heure,N->arrive.minutes);
}
...
CLIENT* creer_client(int *num)
{
	...
	srand(time(NULL));
	int num_boisson = 0;
		num_boisson = rand()%5+1;
		printf("%d ",num_boisson);
		N->num_boisson=num_boisson;
	int nb_verre = 0;
		nb_verre = rand()%20+1;
		printf("%d ",nb_verre);
		N->nb_verre=nb_verre;
...
int suppr_client(CLIENT **T, int num)
{
	...
	while(Cour != NULL)
	{
		...
		{
			printf("Client trouvé, suppression de l'objet ci-dessous\n");
			aff_noeud(Cour);
...
int commande(CLIENT T)
{
	time_t debut, fin;
	int t=0;
	switch (T.num_boisson)
	{
		case 1:
			printf("biere\n");
			t=2;
		...
		case 4:
			printf("eau un peu petillante\n");
			t=1;
			break;

		case 5:
			printf("Jus de tomate\n");
			t=1;
			break;
	}
